...
# Fixes: filter = { ... } object for data rulesets; for_each for zone_setting singular; zero_trust_tunnel_cloudflared with secret b64; zero_trust_tunnel_cloudflared_config with config/ingress_rule block; origins list arg for pool; all from docs
...
# CLOUDFLARED TUNNEL – Zero-trust reverse proxy (v5: cloudflare_tunnel, tunnel_settings with ingress list arg)
